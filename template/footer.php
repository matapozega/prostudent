<footer class="footer">
  <div class="row">
    <div class="small-12 columns">
      <p class="slogan">Pro-Student</p>
      <p class="links">
         <a href="<?php echo $putanjaAPP ;?>index.php"><span>O natječaju</span></a>
		 <a href="<?php echo $putanjaAPP ;?>zeliteotvorititvrtku.php"><span>Želite otvoriti tvrtku?</span></a>
		 <a href="<?php echo $putanjaAPP ;?>povijestprostudenta.php"><span>Povijest Pro-Studenta</span></a>
		 <a href="<?php echo $putanjaAPP ;?>partneri.php"><span>Partneri</span></a>
      </p>
      <p class="copywrite">©FERIT Osijek</p>
    </div>
  </div>
</footer>